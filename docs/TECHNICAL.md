...
The installer checks `latest.yml` through WinHTTP (native, no extra process) and, when a published release is newer than its embedded payload, downloads and installs that instead — so running an older `FleetInstaller.exe` still installs the newest Fleet. The download is digest-verified before extraction. Its window is a single Fleet-branded page in the app's own dark palette: a wordmark header, custom-drawn buttons (GDI+, anti-aliased 8px rounding, matching the app's `.btn` styles), Windows 11 rounded window corners, a dark immersive caption, and native dark-styled folder edit and checkboxes. The content swaps in place — install form, update notice, progress, done — with no wizard pages, and the release-feed check runs while the page settles instead of behind a splash.
