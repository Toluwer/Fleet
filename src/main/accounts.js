 ...
 *
 * Stored accounts keep their session cookie encrypted locally and use it for
 * presence, People joins, and authenticated Roblox launches. New account
 * sign-in is handled by the Tauri shell and passed here after Roblox sets the
 * session cookie.
 *
...
async function addFromCookie(cookie) {
  if (!cookie || typeof cookie !== 'string' || cookie.length < 100) {
    return { ok: false, error: 'Roblox sign-in did not return a usable session.' };
  }
  try {
    ...
    return { ok: true, account: sanitize(account), updated: !!existing, accounts: list() };
  } catch (err) {
...
async function add() {
  return {
    ok: false,
    unavailable: true,
    error: 'Account sign-in must be started by the Tauri shell.',
  };
}
